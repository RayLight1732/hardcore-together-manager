...
本リポジトリの`Dockerfile`はGateのコンテナイメージのみをビルドする。Manager（+hardcoreサーバー）のイメージはManager側リポジトリでは配布されない（Managerのコード自体はJava非依存で、GitHub ReleaseでGoバイナリのみを配布している）ため、デプロイ用リポジトリ（hardcore-together-deploy）側でJavaランタイムと組み合わせてビルドされる（architecture-manager.md 11節）。
