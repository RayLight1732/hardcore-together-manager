...
**`force`の適用範囲**：`force`は`/load`にのみ存在し、「hardcoreの`running`が`true`の場合に拒否する」チェックを免除する。`/start`（`clean`の有無を問わず）・`/deactivate`には`force`の概念が無い——`/start`（`clean`無し）はそもそも`running`を見ないので免除するものが無く、`/start clean`は常に無条件で実行され、`/deactivate`の拒否理由は「既に停止している」の1種類のみだから。3.2節の「アーカイブ実行中は`/start`・`/load`をブロックする」排他制御は`force`・`clean`のいずれでも免除しない（ファイルコピーの最中にセーブフォルダを消すのは別種の危険なので、これは常に守る）。`/start clean`・`/load force`の実行時、退避時点で`running=true`だった（＝進行中の挑戦を実際に破棄する）場合は、退避されるプレイヤーへの通知メッセージを通常の「ワールドリセットのためロビーに戻りました」から「管理者により強制リセットされました」に変える。
...
| `deactivate-rejected` | Manager → Gate | プロセス状態チェックで拒否した場合 | `reason`（「既に停止しています」） |
...
3. Manager：（`load`の場合のみ）`running`チェック・アーカイブ存在チェック → 拒否ならManager → Gate：`start-rejected`/`load-rejected`を返して終了（`start`は`clean:true`の場合、この時点でのチェックは無い——常に受理される）
...
`/start`（`clean`無し。対象状態は常にプロセス停止中、2.1節の④・⑥。プロセスが既に起動中の場合はこの手順に入らず即座に`start-rejected`）：

1. Gate：コマンド受理・権限チェック
2. Gate → Manager：`start`（`clean:false`）送信
3. Manager：プロセス状態チェック（既に起動中/起動処理中か）・ワールド存在チェック → 拒否ならManager → Gate：`start-rejected`を返して終了
4. Manager：プロセス起動（ワールドには触れない）
