...
**受理後に失敗した場合**（`architecture-manager.md`変更履歴末尾参照）：`start-rejected`等の拒否判定はあくまで**受理する前**のチェックであり、受理した後（退避待ち・プロセス停止・ワールド準備・プロセス起動・`ready`待ちのいずれか）で失敗するケースは当初考慮されておらず、`hardcore-ready`・`deactivate-complete`（成功）以外に結果を伝える手段が無かった。このため、実際にhardcoreプロセスが起動直後にクラッシュする（例：ファイル権限エラー）という障害が発生した際、Gate・プレイヤー側からは`evacuate-complete`送信後に何も届かず、コマンドが永久にハングしたように見える、という不具合が実機で見つかった。

この問題を修正するため、`start`・`load`・`deactivate`受理後の失敗を通知する`start-failed`・`load-failed`・`deactivate-failed`（7.2節）を追加した：

- Gateはこれを受信したら、要求者へ`reason`を表示する（例：「サーバーの起動に失敗しました：`<reason>`」）。それまでの`evacuate-request`・`evacuate-complete`のやり取りは無駄にはならない（既に退避は完了しているため、失敗表示だけで状態として矛盾は無い）
- `recovered`（bool）は、Managerが「hardcoreプロセスが実際に動いていないこと」を確認した上でプロセス状態を`停止中`へ戻せたかどうかを示す。`true`なら即座に`/start`等を再試行してよい。`false`の場合、Managerは安全側に倒してプロセス状態を`起動処理中`/`停止処理中`のまま維持する（`/start`・`/start clean`・`/load`・`/deactivate`のいずれも、遷移中は状態に関わらず「処理中です」で拒否され続ける、前述「`force`の適用範囲」）——`recovered:false`の場合、Gateは「手動での確認が必要です」等、再試行だけでは解決しない可能性がある旨も併せて表示すべき
...
- **プレイヤー死亡時**（`LivingDeathEvent`、対象がPlayerかつ`running=true`の場合のみ）：
  1. `LivingDeathEvent`をキャンセルし、イベントハンドラ内で同期的に体力/満腹度をリセットしてスペクテイターへ切り替える。**アイテム・経験値のドロップは発生しない**（`die()`本体が実行されないため）。**キャンセル方式を採る理由**：vanillaのハードコアBAN（`server.isHardcore()`時に死亡プレイヤーが`banned-players.json`へ追加される処理）は、クライアントが死亡画面で「リスポーン」を押した際に送る`PERFORM_RESPAWN`パケットのハンドラ（`ServerGamePacketListenerImpl#handleClientCommand`）側に実装されている。死亡画面自体はクライアントが同期された体力が0以下になったことを検知して表示するため、`LivingDeathEvent`をキャンセルしてダメージ処理中に体力を回復させてしまえば、体力0以下の値がクライアントへ同期されることが無く、死亡画面自体が一度も表示されない。結果として`PERFORM_RESPAWN`パケットも送られず、BAN分岐に到達しない。旧Paper版（`Player#spigot().respawn()`）・旧NeoForge実装（死亡を通常通り進行させ、次tickに`PlayerList#respawn`を直接呼ぶ方式）は、死亡自体は完了させた上でリスポーンだけをクライアント操作無しに完了させる方式だったが、`PlayerList#respawn`は常にベッド/ワールドスポーンへ再配置するため死亡地点に留まれなかった。現行実装（キャンセル方式）はこの問題も解消している
...
**手順4〜7のいずれかで失敗した場合**（`evacuate-complete`が届かない、プロセス停止に失敗、ワールド準備に失敗、プロセス起動に失敗、`ready`が届かない）、Manager → Gate：`start-failed`/`load-failed`（`reason`・`recovered`付き、2.1節「受理後に失敗した場合」）を返して終了する。
...
手順4〜5で失敗した場合（プロセス起動に失敗、`ready`が届かない）、Manager → Gate：`start-failed`（`reason`・`recovered`付き）を返して終了する。`recovered`は、`Process.IsRunning()`でプロセスの生死を確認できた場合に`true`（プロセス停止中へ戻し即座に再試行可能）、確認できなかった場合は`false`（起動処理中のまま、他のコマンドも遷移中ガードで拒否され続ける）。
...
手順4〜6で失敗した場合（`evacuate-complete`が届かない、プロセス停止に失敗）、Manager → Gate：`deactivate-failed`（`reason`・`recovered`付き）を返して終了する。
...
- 【解消】`archive-request`が名前重複でManagerに拒否された場合の即時通知シグナル（`archive-rejected`、`requestId`による相関）をプロトコルとして設計済み（6節・3.2節）。**Manager（Go、本リポジトリ）側はこのプロトコルに追従済み。hardcore MOD（Kotlin、別リポジトリ）側の実装はまだ反映されていない**。特にMOD側は現状`/archive`コマンドの実行スレッド（サーバーメインスレッド）が`archive-complete`受信まで同期的にブロックする実装になっており、`archive-rejected`を活かすには非同期化（コマンドを即座に返し、応答受信時に`source`経由でOPへ結果を通知する設計）も合わせて必要になる（`architecture-neoforge.md`「未着手・既知の課題」参照）
